...
//                SensorManager.AXIS_Y, SensorManager.AXIS_MINUS_X, mR);
     ...
     //   SensorManager.remapCoordinateSystem(mR, SensorManager.AXIS_X, SensorManager.AXIS_Z, mI);
        SensorManager.getOrientation(mR, mOrientation);
        float incl = SensorManager.getInclination(mR);

        final float rad2deg = (float)(180.0f/Math.PI);
        
        if (mCount++ > 0) {
        Vector3f orientation = new Vector3f(incl*rad2deg,mOrientation[2]*rad2deg,mOrientation[0]*rad2deg);
        
      Log.d("orientation", ""+ orientation);
        
           
            mCount = 0;
            Log.d("Compass", "(z)yaw: " + (int)(mOrientation[0]*rad2deg) +
                    "  pitch: " + (int)(mOrientation[1]*rad2deg) +
                    "  (y)roll: " + (int)(mOrientation[2]*rad2deg) +
                    "  (x)incl: " + (int)(incl*rad2deg)
                    );
        }
    }
}
